        ...
        const std::vector<ConvexPolygon*>& regionPath)
{
    ...
    for (size_t i = 0; i < regionPath.size() - 1; ++i) {
        ...
        if (sharedEdgeOpt.has_value()) {
            ...
            // Determine the order of the portal points
            Vector2 direction = Vector2Subtract(next->centroid, current->centroid);
            Vector2 normal = Vector2{-direction.y, direction.x};

            float dotLeft = Vector2DotProduct(Vector2Subtract(sharedEdge.first, current->centroid), normal);
            float dotRight = Vector2DotProduct(Vector2Subtract(sharedEdge.second, current->centroid), normal);

            if (dotLeft > dotRight) {
                std::swap(sharedEdge.first, sharedEdge.second);
                std::cout << "Portal " << i << " swapped for correct ordering.\n";
            }
...
std::optional<Vector2> Pathfinding::findClosestPoint(float x, float y, const NavMesh& navMesh, ConvexPolygon*& regionOut) {
    Vector2 point = Vector2{ x, y };
    ...
    for (const auto& region : navMesh.regions) {
        ...
        for (size_t i = 0; i < region->vertices.size(); ++i) {
            ...
            float distSq = Vector2DotProduct(Vector2Subtract(point, projection), Vector2Subtract(point, projection));
            if (distSq < minDistSq) {
                minDistSq = distSq;
