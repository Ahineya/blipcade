    ...
    handleMouseEvent(event) {
        if (event.type === "mouseMove") {
            ECS.forEachEntity(["ActionMenu"], (entity, actionMenu) => {
                ...
                // const isHovered = Collision.checkCollisionPoint(x, y, 20, 20);
                // We don't have Collision.checkCollisionPoint yet, let's do a manual check:
                const isHovered = event.position.x >= x && event.position.x <= x + 20 && event.position.y >= y && event.position.y <= y + 20

                if (isHovered) {
                    actionMenu.hovered = true;
                } else {
